...
def predict():
    if request.method == "POST":
        ...
        predicted_categories = {
            category: budget for category, budget in zip(X.columns, per_category)
        }
        return render_template(
            "result.html",
            categories_budget=predicted_categories,
            total_budget=predicted_total_spending,
        )
    else:
...
def predict_2():
    if request.method == "POST":
        ...
        remaining_features = [
            feature for feature in features if feature not in top_features
        ]
        ...
        for feature in top_features:
            allocated_amount = remaining_budget / (
                num_remaining_features + len(top_features)
            )
